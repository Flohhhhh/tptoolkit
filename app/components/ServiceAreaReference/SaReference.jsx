                        ...
                        <thead className='text-shark-900 dark:text-shark-200 h-12'>
                            <tr>
                                {table.headings.map((heading, index) => {
                                    return <th className="px-4 py-2" key={index}>{heading}</th>
                                })}
                            </tr>
                        </thead>
                        <tbody>
                            {table.rows.map((row, index) => {
                                return <tr key={index} className='text-shark-700 dark:text-shark-200 odd:bg-shark-50 dark:odd:bg-shark-700/80 border-b border-shark-200 dark:border-shark-600'>
                                    {row.map((cell, index) => {
                                        return <td className="px-4 py-2" key={index}>{cell}</td>
                                    })}
                                </tr>
                            })}
                        </tbody>
