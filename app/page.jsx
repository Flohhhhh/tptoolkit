...
import MileMarker from "./components/MileMarker.jsx";
  ...
  return (
    <main className='flex w-full sm:max-w-2xl sm:mx-auto min-h-screen flex-col items-center justify-center gap-4 p-24 pointer-events-none'>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button
          type='submit'
          className='bg-zinc-800 rounded-md p-2 w-full mt-2 hover:brightness-150 active:scale-95 transition'
        >
          Find
        </button>
      </form>
      <div className='h-96 flex items-center'>
        {loading ? (
          ...
          <div role='status'>
            <svg
              aria-hidden='true'
              className='w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-zinc-100'
              viewBox='0 0 100 101'
              fill='none'
          ...
          <div className='flex flex-col gap-1'>{Locations}</div>
        )}
      </div>
    </main>
  );
